...
import { Home, Compass, MessageCircle, PlusSquare, Instagram, LogOut } from "lucide-react"
...
const Navigation = () => {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 border-r border-border bg-card flex flex-col p-4 z-50">
      {/* Logo */}
      <Link to="/" className="flex items-center gap-3 mb-8 px-2">
        ...
        <span className="text-xl font-bold gradient-text">PostScript</span>
      </Link>
        ...
        {profile && (
          <Link to={`/profile/${profile.uid}`}>
            ...
            >
              ...
              <span className={location.pathname.startsWith("/profile") ? "font-semibold" : ""}>Profile</span>
            </Button>
          </Link>
        )}
      </nav>
