...
const PostCard = ({ post }: PostCardProps) => {
  ...
  const renderMedia = () => {
    ...
    switch (post.post_type) {
      ...
      case "video":
        return <video src={post.media_urls[0]} controls className="w-full h-auto max-h-[600px] rounded-lg" />
      case "carousel":
        return (
      ...
      case "audio":
        return <audio src={post.media_urls[0]} controls className="w-full" />
      default:
        return null
    }
  }
  ...
  const renderContent = () => {
    switch (post.post_type) {
      ...
      default:
        return post.caption && <p className="text-foreground whitespace-pre-wrap">{post.caption}</p>
    }
  }
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" onClick={handleLike} className="hover:text-red-500">
            ...
            {/* Likes count */}
            {likesCount > 0 && <p className="text-sm font-semibold">{likesCount} likes</p>}
            ...
            {post.difficulty && (
              <Badge
                variant={
                  post.difficulty === "easy" ? "default" : post.difficulty === "medium" ? "secondary" : "destructive"
                }
              >
                {post.difficulty}
              </Badge>
